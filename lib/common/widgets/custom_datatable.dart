...
class MyDataCell {
  ...
  static const MyDataCell empty = MyDataCell(SizedBox(width: 0.0, height: 0.0));
...
class MyDataTable extends StatelessWidget {
  ...
  void _handleSelectAll(bool? checked, bool someChecked) {
    ...
    if (onSelectAll != null) {
      ...
      for (final MyDataRow row in rows) {
  ...
  Widget _buildDataCell({
    ...
    label = Container(
      ...
      alignment:
          numeric ? Alignment.centerRight : AlignmentDirectional.centerStart,
      decoration: BoxDecoration(
          border: Border.all(color: ColorConstant.darkBlue, width: 0.4)),
  ...
  Widget build(BuildContext context) {
    ...
    final bool anyRowSelectable =
        rows.any((MyDataRow row) => row.onSelectChanged != null);
    ...
    final Iterable<MyDataRow> rowsWithCheckbox = displayCheckboxColumn
        ? rows.where((MyDataRow row) => row.onSelectChanged != null)
        : <MyDataRow>[];
    final Iterable<MyDataRow> rowsChecked =
        rowsWithCheckbox.where((MyDataRow row) => row.selected);
    ...
    for (int dataColumnIndex = 0;
        ...
        dataColumnIndex += 1) {
      final MyDataColumn column = columns[dataColumnIndex];
...
/// See also:
///
///  * [MyDataTable], which makes use of [TableRowInkWell] when
